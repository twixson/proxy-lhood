...
source("./code/generate_TLETS.R")

#####
# Functions to get list of large lag-h points
get_large_lag_h_pairs <- function(ts, h, set_thresh_val = F, thresh){
  n         <- length(ts)
  ...
  if(!set_thresh_val){
    ...
    large_ind <- which(temp_rad > set_thresh_val)
  }
  return(list(indices = large_ind, points = temp_mat[large_ind, ]))
}

get_all_large_pairs <- function(ts, max_h=20, set_thresh_val = F, 
                                thresh = 0.975, bias_corr = F){
  if(bias_corr){
    temp_mean <- mean(ts)
    n_temp <- length(ts)
  ...
  if(!set_thresh_val){
    temp_out       <- lapply(1:max_h, get_large_lag_h_pairs, ts = ts, 
                             thresh = thresh)
  } else {
    temp_out       <- lapply(1:max_h, get_large_lag_h_pairs, ts = ts,
                             set_thresh_val = set_thresh_val, thresh = thresh)
  }
  indices        <- lapply(temp_out, function(x){x$indices})
  points         <- lapply(temp_out, function(x){x$points})
  names(indices) <- paste0("indices_lag_", 1:max_h)
  names(points)  <- paste0("points_lag_", 1:max_h)
  return(list(indices = indices, points = points))
}

get_radial_thresh <- function(ts, h = 1, thresh = 0.975){
  n         <- length(ts)
  temp_mat  <- matrix(c(ts[1:(n-h)], ts[(1+h):n]), ncol = 2, byrow = F)
  temp_rad  <- apply(temp_mat, 1, function(x){sqrt(sum(x^2))})
  unname(quantile(temp_rad, prob = thresh))
}
...
ma_q_tpdf <- function(thetas, max_lag. = 20){
  q <- length(thetas)
  if(max(thetas) < 0){
    return(rep(min(thetas), max_lag.))
  } else {
    sigmas <- rep(0, max_lag.)
    thetas <- c(1, thetas, rep(0, max_lag.))
    for(i in 1:q){
      sigmas[i] <- sum(pmax(thetas[1:(q+1)], 0) * 
                         pmax(thetas[(i+1):(i+q+1)], 0)) / 
        sum(pmax(thetas, 0)^2) # this is sigma_0 and thus forces the scale to be 1
    }
    # if(thetas[q+1] < 0) {
    #   sigmas[q] <- min((-thetas[q+1]) + 0.1, 0.999)
    #   for(l in q:1){
    #     if(thetas[l] < 0){
    #       sigmas[l] <- min((-thetas[l]) + 0.1, 0.999)
    #     } else {
    #       break
    #     }
    #   }
...
get_lambda <- function(sigma){
  ifelse(is.finite(sqrt(-2 * log(sigma))), sqrt(-2 * log(sigma)), 1000)
}

get_grad <- function(sigma){
...
# h_l2 <- function(v, l){
#   if(l < 0.0033){l = 0.0033}
#   vj    <- sqrt(1-v^2)
#   logi  <- log(v / vj)
#   logj  <- log(vj / v)
#   phii  <- dnorm(l - logi/l)
#   phij  <- dnorm(l - logj/l)
#   ti    <- (l^2 + logi)/(l^3 * v^3 * vj) 
#   tj    <- (l^2 + logj)/(l^3 * v * vj^3) 
#   return(0.5 * v * (ti * phii + tj * phij))
# }
# 
# hr_tpdm <- function(lambda){
#   integrate(h_l2, lower = 0, upper = 1, l = lambda)$value
# }
# 
# hr_optim <- function(lam, target){
#   abs(hr_tpdm(lam) - target)
# }
...
#####
# log-likelihood contributions (negated)
...
get_ARMA11_ll_from_list <- function(theta_phi, points_list, 
  ...
  # print(paste0("temp theta: ", theta_phi[1], "     temp phi: ", theta_phi[2]))
...
optim_MAq_multiple <- function(theta_init, points_list, min_q = 1, max_q = 20, 
                               max_lag=20, weights = 0, omethod = "L-BFGS-B"){
  if(length(theta_init) > 1){
    stop("ERROR: different initial values not supported yet")
  }
  
  fit_list <- list()
  list_ind <- 1
  if(min_q == 1){
    fit_list[[1]] <- optim(get_MAq_from_list, 
                           par = theta_init, 
                           points_list = points_list, 
                           max_lag = max_lag, 
                           weights = weights, 
  ...
  for(q in max(2, min_q):max_q){
    temp_out <- optim(get_MAq_from_list, 
                      par = rep(theta_init, q), 
                      points_list = points_list, 
                      max_lag = max_lag, 
                      weights = weights, 
                      hessian = T, 
    ...
    fit_list[[list_ind]] <- temp_out
    list_ind <- list_ind + 1
    if(q %% 5 == 0){
      print(paste0("~~~ Order ", q, " fitting complete ~~~"))
    }
  }
  names(fit_list) <- paste0("MA", min_q:max_q)
  return(fit_list)
}
...
optim_MAq_sequentially <- function(max_q, points_list, max_lag = 20, weights = 0, 
  ...
  # starting with 0.5 behaves better than 0, zero keeps some values at zero or 1 more often
                      points_list = points_list,
                      max_lag = max_lag,
                      weights = weights, 
                      hessian = T, 
...
HR_score_contribution <- function(x, lambda.){ # eq (6) in manuscript
  ...
  dVi  <- 
    -2 * pnorm(aij)/(xi^3)- 
    dnorm(aij)/(lambda. * (xi^3)) + 
    dnorm(aji)/(lambda. * xi * (xj^2))
  dVj  <- 
    -2 * pnorm(aji)/(xj^3) - 
    dnorm(aji)/(lambda. * (xj^3)) + 
    dnorm(aij)/(lambda. * (xi^2) * xj)
  dVidVj <- 
    (-lambda.^2 - lij) * dnorm(aij)/(lambda.^3 * (xi^3) * xj)  + 
    (-lambda.^2 - lji) * dnorm(aji)/(lambda.^3 * xi * (xj^3))
  dldVi <-
    (-2 * dnorm(aij)/(xi^3)) * (1 + lij/(lambda.^2)) + 
    (lambda.^2 + 1 - lij^2/(lambda.^4)) * dnorm(aij)/(xi^3) - 
    (lambda.^2 + 1 - lji^2/(lambda.^4)) * dnorm(aji)/(xi * (xj^2))
  dldVj <- 
    (-2 * dnorm(aji)/(xj^3)) * (1 + lji/(lambda.^2)) + 
    (lambda.^2 + 1 - lji^2/(lambda.^4)) * dnorm(aji)/(xj^3) - 
    (lambda.^2 + 1 - lij^2/(lambda.^4)) * dnorm(aij)/((xi^2) * xj)
  dldVidVj <- 
...
ddtheta_arma11_theta <- function(phi, theta, max_lag.=20){
  ...
  if(phi > 0){
    if(phi + theta > 0){ # case 1
      num1 <- phi^((1:max_lag.)-1) * (phi^2 - 1) * (theta^2 - 1)
      den1 <- (theta^2 + 2*theta*phi + 1)^2
      dval <- num1/den1
    } else { # case 2
      dval <- rep(0, max_lag.)
    }
    ...
    num1[evens] <- 2 * phi^evens * (phi + theta)
    den1        <- (phi + theta)^2 - phi^4 + 1
    num2[evens] <- 2 * phi^evens * (phi + theta)^3
    den2        <- den1^2
    num1[odds]  <- phi^(odds - 1) * (1 - phi^4)
    num2[odds]  <- 2 * phi^(odds - 1) * (phi + theta)^2 * (1 - phi^4)
    dval        <- num1/den1 - num2/den2
  } else { # case 4/5
    evens <- seq(2, max_lag., by = 2)
    odds  <- seq(1, max_lag., by = 2)
    num1[evens] <- phi^(evens - 1) * (phi^4 - 1) * (phi^2 * theta^2 - 1)
    den1        <- (theta^2 * phi^2 + 2 * phi^3 * theta + 1)^2
    num1[odds]  <- 0
    num2[odds]  <- 0
    dval <- num1/den1
  }
...
ddtheta_arma11_phi <- function(phi, theta, max_lag.=20){
  ...
  if(phi > 0){
    if(phi + theta > 0){ # case 1
      num1 <- 
        (1:max_lag. - 1) * phi^(1:max_lag. - 2)*(phi + theta)*(phi*theta + 1) + 
        phi^(1:max_lag. - 1)*theta*(phi + theta) + 
        phi^(1:max_lag. - 1)*(phi*theta + 1)
      den1 <- theta^2 + 2*theta*phi + 1
      num2 <- 2 * phi^(1:max_lag. - 1)*theta*(phi + theta)*(phi*theta + 1)
      den2 <- den1^2
      dval <- num1/den1 - num2/den2
    } else { # case 2
      dval <- rep(0, max_lag.)
    ...
    evens <- seq(2, max_lag., by = 2)
    odds  <- seq(1, max_lag., by = 2)
    num1[evens] <- evens * phi^(evens - 1)*(phi + theta)^2 + 
      2 * phi^evens * (phi + theta)
    den1        <- (phi + theta)^2 - phi^4 + 1
    num2[evens] <- phi^evens * (phi + theta)^2 * (2*phi + 2*theta + 4*phi^3)
    den2        <- den1^2
    num1[odds]  <- (odds - 1) * phi^(odds - 2) * (phi + theta) * (1 - phi^4) + 
      phi^(odds - 1) * (1 - phi^4) -
      4 * phi^(odds + 2) * (phi + theta)
    num2[odds]  <- phi^(odds - 1) * (phi + theta) * (1 - phi^4) * 
      (2*phi + 2*theta + 4*phi^3)
    dval        <- num1/den1 - num2/den2
  } else { # case 4/5
    evens <- seq(2, max_lag., by = 2)
    odds  <- seq(1, max_lag., by = 2)
    num1[evens] <- 
      (evens - 1) * phi^(evens - 2) * (phi + theta) * (phi^3 * theta + 1) + 
      phi^(evens - 1) * (phi^3 * theta + 1) + 
      3 * phi^(evens + 1) * theta * (phi + theta)
    den1        <- theta^2 * phi^2 + 2 * phi^3 * theta + 1
    num2[evens] <- phi^(evens - 1) * (phi + theta) * (phi^3 * theta + 1) * 
      (2 * phi * theta^2 + 6 * phi^2 * theta)
    den2        <- den1^2
    num1[odds]  <- 0
    num2[odds]  <- 0
    dval <- num1/den1 + num2/den2
...
scores_list <- function(points_list, 
  ...
  ## uncomment this if you want to multiply by number of points... 
  # n_m      <- rep(0, M)
  for(s in 1:M){
    for(h in 1:max_lag){
      if(length(which(
        ...
        ddll[s, h] <- 
          HR_score_contribution(x = 
                                ...
                                lambda = lambdas[h])
        ## uncomment to this if you want to multiply by number of points... 
        # n_m[s] <- n_m[s] + 1
        ...
        ddll[s, h] <- 
          HR_score_pairwise(data = 
                            ...
                            lambda = lambdas[h])
        ## uncomment to this if you want to multiply by number of points... 
        # n_m[s] <- n_m[s] + length(which(
        #   indices[[h]] %in% M_splits[s]:(M_splits[s+1]-1)))
      } else {
        ddll[s, h] <- 0
        } 
    }
  }
  ## Switch to this if you want to multiply by number of points... 
  # print(n_m)
  # n_m * (ddll %*% ddts) / max_lag
  ddll %*% ddts
}
...
# Compute the CLAIC penalty from the score matrix and hessian
get_penalty <- function(scores_matrix, hess_mat){
  CLAIC_penalty <- sum(diag(cov(scores_matrix) %*% solve(hess_mat)))
  return(CLAIC_penalty)
}
...
get_scores <- function(library_fits, ts_list, split_length, ts_length, 
  ...
  models <- ifelse(substr(model_names, 1, 2) == "MA", "maq", model_names)
  models <- ifelse(substr(model_names, 1, 4) == "ARMA", "arma11", models)
  models <- ifelse(substr(model_names, 1, 3) == "AR1", "ar1", models)
...
# Scripts to get AN CI's 
    ### DO NOT USE, NO THEORY, BAD COVERAGE
...
get_CIs <- function(params, cov_mat, t_df){
  print("**Heuristic intervals at best. These intervals have terrible coverage.**")
...
# Alternative ways to split into subseries for the scores: 

# This splits into subseries with the same number of large points so that the
#    scores are derivatives of likelihood surfaces from the same number of obs. 
scores_list_new <- function(points_list, 
                            indices, 
                            num_subseries, 
                            model, 
                            Theta_hat, 
  ...
  ddll     <- matrix(NA, nrow = num_subseries, ncol = max_lag)
  for(h in 1:max_lag){
    index_list <- split(1:(dim(points_list[[h]])[1]), 
                        cut(1:(dim(points_list[[h]])[1]), num_subseries))
    for(s in 1:num_subseries){
      ddll[s, h] <- 
        HR_score_pairwise(data = points_list[[h]][index_list[[s]], ], 
                          lambda = lambdas[h])
    }
  }
  ddll %*% ddts
...
# This computes the scores from random subsets of large points. This violates
#    the independence assumptions with the potential benefit of obscuring the 
#    non-stationarity in seasons. 
scores_list_random <- function(points_list, 
                               indices, 
                               num_subseries, 
                               model, 
                               Theta_hat, 
  ...
  ddll     <- matrix(NA, nrow = num_subseries, ncol = max_lag)
  for(h in 1:max_lag){
    n_large_points <- dim(points_list[[h]])[1]
    index_list <- split(sample(1:n_large_points, n_large_points, replace = F),
                        cut(1:n_large_points, num_subseries))
    for(s in 1:num_subseries){
      ddll[s, h] <- 
        HR_score_pairwise(data = points_list[[h]][index_list[[s]], ], 
                          lambda = lambdas[h])
...
scores_list_overlap <- function(points_list, 
                                indices, 
                                num_subpoints, 
                                model, 
                                Theta_hat, 
  ...
  n_points <- min(sapply(points_list, function(x)dim(x)[1]))
  num_subs <- n_points - num_subpoints + 1
  ddll     <- matrix(NA, nrow = num_subs, ncol = max_lag)
  for(h in 1:max_lag){
    for(s in 1:num_subs){
      temp_inds  <- s:(s + num_subpoints - 1)
      ddll[s, h] <- HR_score_pairwise(data = points_list[[h]][temp_inds, ], 
                                      lambda = lambdas[h])
    }
  }
  ddll %*% ddts
...
get_scores_new <- function(library_fits, ts_list, num_subseries, ts_length){
  model_names       <- names(library_fits)
  no_penalty <- CLAIC <- CLBIC <- numeric(length(library_fits))
  names(no_penalty) <- model_names
  ...
  for(i in 1:length(library_fits)){
    no_penalty[i]    <- library_fits[[i]]$value
    temp_scores      <- scores_list_new(points_list = ts_list$points, 
                                        indices = ts_list$indices, 
                                        num_subseries = num_subseries, 
                                        model = models[i], 
                                        Theta_hat = library_fits[[i]]$par)
    if(is.null(library_fits[[i]]$hessian)){
      print(paste0("ERROR: Model ", model_names[i], 
...
get_scores_random <- function(library_fits, ts_list, num_subseries, ts_length){
  model_names       <- names(library_fits)
  no_penalty <- CLAIC <- CLBIC <- numeric(length(library_fits))
  names(no_penalty) <- model_names
  models            <- c(rep("maq", length(model_names)-2), 
  ...
  for(i in 1:length(library_fits)){
    no_penalty[i]    <- library_fits[[i]]$value
    temp_scores      <- scores_list_random(points_list = ts_list$points, 
                                           indices = ts_list$indices, 
                                           num_subseries = num_subseries, 
                                           model = models[i], 
                                           Theta_hat = library_fits[[i]]$par)
...
get_scores_overlap <- function(library_fits, ts_list, num_subpoints, ts_length){
  model_names       <- names(library_fits)
  no_penalty <- CLAIC <- CLBIC <- numeric(length(library_fits))
  names(no_penalty) <- model_names
  models            <- c(rep("maq", length(model_names)-2), 
  ...
  for(i in 1:length(library_fits)){
    no_penalty[i]    <- library_fits[[i]]$value
    temp_scores      <- scores_list_overlap(points_list = ts_list$points, 
                                            indices = ts_list$indices, 
                                            num_subpoints = num_subpoints, 
                                            model = models[i], 
                                            Theta_hat = library_fits[[i]]$par)
    if(is.null(library_fits[[i]]$hessian)){
      print(paste0("ERROR: Model ", model_names[i], 
