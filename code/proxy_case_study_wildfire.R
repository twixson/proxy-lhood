...
fire_data <- readRDS("./data/ERA5_CO_timeseries_data_noBiasCorr_dg071526.rds")

#### EARLY (past climate)
set.seed(2293)
early_ts  <- c(fire_data$early)
...
early_MA_fits <- optim_MAq_multiple(theta_init = 0.1, max_q = 20, 
                                        points_list = out_early$points)

fitted_models_e <- early_MA_fits
fitted_models_e$AR1 <- early_AR1_fit
fitted_models_e$ARMA11 <- early_ARMA11_fit
scores_early <- get_scores(library_fits = fitted_models_e, 
                           ts_list = out_early, 
                           split_length = 153, 
                           ts_length = 3060)
...
p_early <- ggplot(data = tpdf_tibble_early, aes(x = lag, y = emp_tpd)) +
  ...
  geom_segment(mapping = aes(xend = lag, yend = 0), lwd = 0.75, col = "orange") +
  ...
  annotate(geom = "text", x=Inf,y=Inf, hjust = 1, vjust = 10,
           label = "fitted MA(15)", color = "grey62") +
  annotate(geom = "text", x=Inf,y=Inf, hjust = 1, vjust = 12,
           label = "fitted MA(20)", color = "#49306B") + # #D55E00
  labs(x = "Lag", y = "TPD") +
  scale_x_continuous(breaks = 0:20) +
  theme_minimal()
p_early
# ggsave(p_early, filename = "./plots/early_ERA5CO_tpdFitted.png",
       width = 8, height = 2.5, units = "in")


######## LATE
set.seed(20398)
late_ts   <- c(fire_data$late)
...
late_AR1_fit <- optim_AR1_from_list(phi = 0.1, points_list = out_late$points)
late_AR1_tpdf <- ar_1_tpdf(phi = late_AR1_fit$par)
late_MA_fits <- optim_MAq_multiple(theta_init = 0.1, max_q = 20, 
                                       points_list = out_late$points)

fitted_models_late <- late_MA_fits
fitted_models_late$AR1 <- late_AR1_fit
fitted_models_late$ARMA11 <- late_ARMA11_fit

score_mat_late <- scores_list(points_list = out_late$points, 
                               indices = out_late$indices, 
                               split_length = 153, 
                               ts_length = 3060, 
                               model = "arma11", 
                               Theta_hat = fitted_models_late$ARMA11$par)

scores_late <- get_scores(library_fits = fitted_models_late, ts_list = out_late, 
...
p_late <- ggplot(data = tpdf_tibble_late, aes(x = lag, y = emp_tpd)) +
  ...
  theme_minimal()
p_late
# ggsave(p_late, filename = "./plots/late_ERA5CO_tpdFitted.png",
       width = 8, height = 2.5, units = "in")
